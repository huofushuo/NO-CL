...
def create_task_composition(class_nums, base_class, num_tasks, fixed_order=False):
    classes_per_task_online = (class_nums - base_class) // (num_tasks - 1)
    classes_per_task = {}
    for num in range(num_tasks):
        if num == 0:
            classes_per_task[num] = base_class
        else:
            classes_per_task[num] = classes_per_task_online
    label_array = np.arange(0, class_nums)
    if not fixed_order:
    ...
    for tt in range(num_tasks):
        # tt_offset = tt * classes_per_task[tt]
        task_labels.append(list(label_array[begin:begin + classes_per_task[tt]]))
        begin += classes_per_task[tt]
        print('Task: {}, Labels:{}'.format(tt, task_labels[tt]))
    return task_labels
    # labels = task_labels[1:]
    # return labels
