    ...
    for run in range(params.num_runs):
        ...
        if params.online:
            for i, (x_train, y_train, labels) in enumerate(data_continuum):
                ...
                if params.agent in ['DSR']:
                    prototype = agent.train_learner(x_train, y_train, session=i)
                    acc_array = agent.evaluate(test_loaders, prototype)
                else:
                    agent.train_learner(x_train, y_train, session=i)
            ...
            if params.data == 'cifar100':
                ...
                total_class = 50
            novel_class = (total_class - params.base_class)//(params.num_tasks-1)
            weight = [params.base_class, novel_class, novel_class, novel_class, novel_class, novel_class, novel_class, novel_class, novel_class, novel_class, novel_class]
            if params.resume == True:
                for i in range(params.num_tasks):
                    print("-----------session {}-----------acc {}-----------train time {}".format(i, np.sum((np.multiply(tmp_acc[i], weight))/(params.base_class+i*novel_class)), run_end - run_start))
                print(
                    "-----------run {}-----------avg_end_acc {}-----------train time {}".format(run, np.sum((np.multiply(tmp_acc[-1], weight))/100), run_end - run_start))
                print(
                    "-----------run {}-----------avg_end_acc(w/o base) {}-----------train time {}".format(run, np.mean(tmp_acc[-1][1:]), run_end - run_start))
            else:
                print(
                    "-----------run {}-----------avg_end_acc(w/ base) {}-----------train time {}".format(run, np.sum((np.multiply(tmp_acc[-1], weight))/100), run_end - run_start))
