import torch
from models.resnet import Reduced_ResNet18, SupConResNet,Reduced_ResNet18_DVC, PCResNet
...
def setup_architecture(params):
    ...
    if params.agent in ['DSR']:
        if params.data == 'mini_imagenet':
            return PCResNet(avg=640, head=params.head, pre_class=params.base_class)
        if params.data == 'core50':
            return PCResNet(avg=2560, head=params.head, pre_class=params.base_class)
        return PCResNet(head=params.head, pre_class=100)
