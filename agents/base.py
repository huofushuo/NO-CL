...
class ContinualLearner(torch.nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self, model, opt, params):
        ...
        self.inner_epoch = params.inner_epoch
        self.base_epoch = params.base_epoch
        self.base_class = params.base_class
        self.batch = params.batch
        self.base_batch = params.base_batch
    ...
    def criterion(self, logits, labels):
        ...
        elif self.params.agent in ['SCR', 'SCP', 'DSR']:
            SC = SupConLoss(temperature=self.params.temp)
            return SC(logits, labels)
        else:
            return ce(logits, labels)

    def criterion_ce(self, logits, labels):
        labels = labels.clone()
        ce = torch.nn.CrossEntropyLoss(reduction='mean')
        return ce(logits, labels)
    ...
    def evaluate(self, test_loaders, prototype=None):
        self.model.eval()
        acc_array = np.zeros(len(test_loaders))
        if self.params.agent in ['PC']:
            exemplar_means = dict(zip(prototype['class_label'], torch.tensor(np.array(prototype['class_mean']))))
            # exemplar_means = dict(zip(prototype['class_label'], torch.tensor(np.array(prototype['class_mean_sign']))))

        elif self.params.trick['ncm_trick'] or self.params.agent in ['ICARL', 'SCR', 'SCP']:
            exemplar_means = {}
            ...
            for task, test_loader in enumerate(test_loaders):
                ...
                for i, (batch_x, batch_y) in enumerate(test_loader):
                    ...
                    if self.params.trick['ncm_trick'] or self.params.agent in ['ICARL', 'SCR', 'SCP']:
                        ...
                        if self.params.agent not in ['PC']:
                        ...
                        means = maybe_cuda(means.transpose(1, 2))
                    ...
                    elif self.params.agent in ['DSR']:
                        feature = self.model.features(batch_x)  # (batch_size, feature_size)
                        # metric: euclidean, cosine, l2, l1
                        # feature = torch.nn.functional.normalize(feature.cpu(), p=2, dim=-1)
                        # means = torch.nn.functional.normalize(means.cpu(), p=2, dim=-1)
                        means = torch.stack([exemplar_means[cls] for cls in self.old_labels])  # (n_classes, feature_size)
                        # pairwise_distance = pairwise_distances(np.asarray(torch.tanh(feature).cpu()), np.asarray(torch.tanh(means).cpu()), metric='cosine')
                        pairwise_distance = pairwise_distances(np.asarray(feature.cpu()), np.asarray(means.cpu()), metric='cosine')
                        pred_label = np.argmin(pairwise_distance, axis=1)

                        correct_cnt = (np.array(self.old_labels)[pred_label.tolist()] == batch_y.cpu().numpy()).sum().item() / batch_y.size(0)
                        # correct_cnt = maybe_cuda(correct_cnt, self.cuda)
