...
class ExperienceReplay(ContinualLearner):
    ...
    def train_learner(self, x_train, y_train, session):
        ...
        # save base pretrain
        model_dir = './pretrained/aser/base_{}classes_{}epoches_{}.pth'.format(self.base_class, self.base_epoch,self.data)
                ...
                for i, batch_data in enumerate(train_loader):
                    ...
                    for j in range(self.mem_iters):
                        ...
                        if mem_x.size(0) > 0:
                            ...
                            if self.params.trick['kd_trick_star']:
                                loss_mem = 1 / ((self.task_seen + 1) ** 0.5) * loss_mem + \
                                           (1 - 1 / ((self.task_seen + 1) ** 0.5)) * self.kd_manager.get_kd_loss(
                                    mem_logits,
                                    mem_x)
                            # update tracker
                            losses_mem.update(loss_mem, mem_y.size(0))
